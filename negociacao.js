...
let simulationCheckerInterval = null;
...
function initNegociacao() {
    ...
    // Inicia o verificador de simulação
    iniciarVerificadorSimulacao();
    
    console.log('✅ Módulo de negociação inicializado com sucesso');
}

// Função para iniciar verificador de simulação
function iniciarVerificadorSimulacao() {
    // Limpa intervalo anterior se existir
    if (simulationCheckerInterval) {
        clearInterval(simulationCheckerInterval);
    }
    
    // Inicia verificação do localStorage para simulação
    simulationCheckerInterval = setInterval(() => {
        try {
            // Verifica pedido simulado
            const orderEventTimestamp = localStorage.getItem('simulatedOrderEvent');
            if (orderEventTimestamp) {
                const orderData = localStorage.getItem('simulatedOrder');
                if (orderData) {
                    // Limpa flag para evitar processamento duplicado
                    localStorage.removeItem('simulatedOrderEvent');
                    
                    // Processa o pedido simulado
                    processarPedidoSimulado(JSON.parse(orderData));
                }
            }
            
            // Verifica disputa simulada
            const disputeEventTimestamp = localStorage.getItem('simulatedDisputeEvent');
            if (disputeEventTimestamp) {
                const disputeData = localStorage.getItem('simulatedDispute');
                if (disputeData) {
                    // Limpa flag para evitar processamento duplicado
                    localStorage.removeItem('simulatedDisputeEvent');
                    
                    // Processa a disputa simulada
                    processarDisputaSimulada(JSON.parse(disputeData));
                }
            }
        } catch (error) {
            console.error('❌ Erro ao verificar simulação:', error);
        }
    }, 1000); // Verifica a cada 1 segundo
    
    console.log('✅ Verificador de simulação iniciado');
}

// Função para processar pedido simulado
function processarPedidoSimulado(orderData) {
    try {
        console.log('📦 Pedido simulado recebido:', orderData);
        
        // Emite um alerta para o usuário
        showToast('Novo pedido simulado recebido! ID: ' + orderData.id, 'info');
        
        // Processa o pedido da mesma forma que um pedido normal
        displayOrder(orderData);
        
        // Notifica o simulador que o pedido foi recebido
        localStorage.setItem('simulatedOrderReceived', 'true');
    } catch (error) {
        console.error('❌ Erro ao processar pedido simulado:', error);
    }
}

// Função para processar disputa simulada
function processarDisputaSimulada(disputeData) {
    try {
        console.log('🤝 Disputa simulada recebida:', disputeData);
        
        // Processa a disputa da mesma forma que uma disputa normal
        processarEventoDisputa(disputeData);
    } catch (error) {
        console.error('❌ Erro ao processar disputa simulada:', error);
    }
}
...
async function aceitarDisputa(disputeId) {
    try {
        ...
        const isSimulation = activeDisputes.some(d => d.disputeId === disputeId && d.simulation === true);
        
        if (isSimulation) {
            // Para simulação, salva a resposta no localStorage
            response = {
                action: 'ACCEPT',
                disputeId: disputeId
            };
            
            // Salva no localStorage para o simulador capturar
            localStorage.setItem('simulatedDisputeResponse', JSON.stringify(response));
            console.log('✅ Resposta de simulação enviada:', response);
        } else {
            // Para API real, faz a chamada HTTP
            response = await makeAuthorizedRequest(`/order/v1.0/disputes/${disputeId}/accept`, 'POST');
...
async function proporAlternativa(disputeId, alternativeId) {
    try {
        ...
        if (isSimulation) {
            ...
            // Adiciona valor de reembolso personalizado se existir
            if (body && body.value) {
                response.customRefundValue = body.value;
            }
...
function exibirModalNegociacao(dispute) {
    ...
    // Marca como simulação se for o caso
    if (dispute.simulation) {
        console.log('🧪 Esta é uma disputa simulada');
    }
...
async function rejeitarDisputa(disputeId) {
    try {
        ...
        if (isSimulation) {
            ...
            // Para API real, faz a chamada HTTP
            response = await makeAuthorizedRequest(`/order/v1.0/disputes/${disputeId}/reject`, 'POST');
